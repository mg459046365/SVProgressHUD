...
- (void)updateHUDFrame {
    ...
    if (hudHeight > self.minimumSize.height && self.multiLineCornerRadius > 0) {
        self.hudView.layer.cornerRadius = self.multiLineCornerRadius;
    } else {
...
- (UIVisualEffectView *)hudView {
    ...
    // Update styling
    // Note: cornerRadius is set in updateHUDFrame based on content height
...
- (void)setMultiLineCornerRadius:(CGFloat)cornerRadius {
    if (!_isInitializing) {
        _multiLineCornerRadius = cornerRadius;
    }
}
